...
模块化的目的是能够用不同作者和来源的代码模块化组装成大型程序，而不会产生命名冲突。由于打包工具的支持，**基于闭包的模块化在实践中称为最常用的模块化形式**，核心是沿用了 Node 的`require()`函数。事实上 ES6 已经使用`import`和`export`关键字定义了自己的模块化格式，但在实践中还是要依赖打包工具。
